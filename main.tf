resource "kubernetes_service" "dgraph-zero-public" {
  metadata {
...
resource "kubernetes_config_map" "dgraph_zero_command" {
  ...
  data = {
    "dgraph_zero.sh" = templatefile("${path.module}/templates/zero.tpl", {prefix = var.prefix, namespace = var.namespace, replicas = var.replicas})
  }
}
